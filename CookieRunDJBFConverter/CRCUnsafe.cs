...
/// </summary>
public unsafe static class CRCUnsafe
{
    private const int UNROLL = 4;
    private const int SLICE = 16;
    private const int BYTES_AT_ONCE = SLICE * UNROLL;

    public static uint Crc32(byte[] data, uint crc = 0)
    {
        fixed (byte* pData = data)
            return Crc32(pData, data.Length, crc);
    }

    public static uint Crc32(byte* dataPtr, int count, uint crc)
    {
        crc = ~crc;

        fixed (uint* lookup = Crc32Lookup)
        {
            var uintPtr = (uint*)dataPtr;
            var uintEndPtr = uintPtr + count / BYTES_AT_ONCE * SLICE;

            uint u1, u2, u3, u4;

            while (uintPtr < uintEndPtr)
            {
                for (var unrolling = 0; unrolling < UNROLL; unrolling++)
                {
                    u1 = *uintPtr++ ^ crc;
                    u2 = *uintPtr++;
                    u3 = *uintPtr++;
                    ...
                    crc = (lookup + 0x000)[(u4 >> 24) & 0xFF] ^
                          (lookup + 0x100)[(u4 >> 16) & 0xFF] ^
                          (lookup + 0x200)[(u4 >> 08) & 0xFF] ^
                          (lookup + 0x300)[(u4 >> 00) & 0xFF] ^
                          (lookup + 0x400)[(u3 >> 24) & 0xFF] ^
                          (lookup + 0x500)[(u3 >> 16) & 0xFF] ^
                          (lookup + 0x600)[(u3 >> 08) & 0xFF] ^
                          (lookup + 0x700)[(u3 >> 00) & 0xFF] ^
                          (lookup + 0x800)[(u2 >> 24) & 0xFF] ^
                          (lookup + 0x900)[(u2 >> 16) & 0xFF] ^
                          (lookup + 0xA00)[(u2 >> 08) & 0xFF] ^
                          (lookup + 0xB00)[(u2 >> 00) & 0xFF] ^
                          (lookup + 0xC00)[(u1 >> 24) & 0xFF] ^
                          (lookup + 0xD00)[(u1 >> 16) & 0xFF] ^
                          (lookup + 0xE00)[(u1 >> 08) & 0xFF] ^
                          (lookup + 0xF00)[(u1 >> 00) & 0xFF];
                }
            }

            // remaining bytes use standard algorithm
            for (var i = count / BYTES_AT_ONCE * BYTES_AT_ONCE; i < count; i++)
                crc = (crc >> 8) ^ *(lookup + ((crc & 0xFF) ^ dataPtr[i]));
        }
